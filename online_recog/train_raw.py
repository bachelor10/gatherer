...
from keras.layers import LSTM, Bidirectional, BatchNormalization, Activation, Embedding, Masking
...
class InkMLSequence(Sequence): # hint: https://gist.github.com/alxndrkalinin/6cc4228e9178ec4af7b2696a0d1ad5a1
	...
	def __init__(self, batch_size=1):  # batch size is one because one matrix is several traces
		...
		self.x_name, self.y_name = populate_data_names(self.file.filename), populate_data_names(self.class_file.filename)
	...
	def __getitem__(self, ix):
		batch_x = self.file[self.x_name[ix]][()] # self.file[self.x_name[ix * self.batch_size:(ix + 1) * self.batch_size]]
		batch_y = self.class_file[self.y_name[ix]][()] # trailing [()] https://stackoverflow.com/questions/10274476/how-to-export-hdf5-file-to-numpy-using-h5py
